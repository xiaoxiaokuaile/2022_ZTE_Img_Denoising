...
class B3_UNet1(nn.Module):
    def __init__(self, in_channels=4, out_channels=4, is_deconv=True, is_batchnorm=False):
        super(B3_UNet1, self).__init__()
        pr_model = EfficientNet.from_pretrained('efficientnet-b3',weights_path='./model/PreModel/efficientnet-b3-5fb5a3c3.pth')
        ...
        self.cnn4 = nn.Sequential(
            pr_model._blocks[8],  # 96 (2,2) (38,38)->(19,19)
            pr_model._blocks[9],  # 96
    ...
    def forward(self, inputs):
        ...
        # down
        conv1 = self.conv1(padded_image)  # torch.Size([1, 24, 512, 512])

        conv2 = self.cnn2(conv1)          # torch.Size([1, 32, 256, 256])
        conv3 = self.cnn3(conv2)          # torch.Size([1, 48, 128, 128])
        conv4 = self.cnn4(conv3)          # torch.Size([1, 136, 64, 64])
        center = self.cnn5(conv4)         # torch.Size([1, 384, 32, 32])
        # up
    ...
    def save(self, circle):
        name = "./weights/B3_UNet1" + str(circle) + ".pth"
...
if __name__ == "__main__":
    ...
    model = EfficientNet.from_pretrained('efficientnet-b3',weights_path='./PreModel/efficientnet-b3-5fb5a3c3.pth')
    model1 = nn.Sequential(
        model._conv_stem,   # 40 (2,2) torch.Size([1, 40, 256, 256])
        model._bn0,         # 40
        model._blocks[0],   # 24  torch.Size([1, 24, 256, 256])
        model._blocks[1],   # 24
        model._blocks[2],   # 32 (2,2) torch.Size([1, 32, 128, 128])
        model._blocks[3],   # 32
        model._blocks[4],   # 32
        model._blocks[5],   # 48 (2,2) torch.Size([1, 48, 64, 64])
        model._blocks[6],   # 48
        model._blocks[7],   # 48
        model._blocks[8],   # 96 (2,2) torch.Size([1, 96, 32, 32])
        model._blocks[9],   # 96
        model._blocks[10],  # 96
        model._blocks[11],  # 96
        model._blocks[12],  # 96
        model._blocks[13],  # 136  torch.Size([1, 136, 32, 32])
        model._blocks[14],  # 136
        model._blocks[15],  # 136
        model._blocks[16],  # 136
        model._blocks[17],  # 136
        model._blocks[18],  # 232 (2,2) torch.Size([1, 232, 16, 16])
        model._blocks[19],  # 232
        model._blocks[20],  # 232
        model._blocks[21],  # 232
        model._blocks[22],  # 232
        model._blocks[23],  # 232
        model._blocks[24],  # 384  torch.Size([1, 384, 16, 16])
