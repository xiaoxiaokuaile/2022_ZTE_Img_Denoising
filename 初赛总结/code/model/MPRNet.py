...
class MPRNet(nn.Module):
    ...
    def __init__(self, in_c=4, out_c=4, n_feat=80, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3,
    ...
    def save(self, circle):
        name = "./weights/MPRNet" + str(circle) + ".pth"
...
if __name__ == '__main__':
    net = MPRNet()
    input_ = torch.Tensor(2, 4, 256, 256)
    out = net(input_)
    print(out[0].shape)

    from thop import profile
    input = torch.randn(1, 4, 256, 256)
    flops, params = profile(net, inputs=(input,))  # 13396100.0  13M
    #  2000
